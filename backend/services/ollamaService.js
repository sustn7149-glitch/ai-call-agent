...
/**
 * Format STT text into conversation format: 상담원/고객 turns
 */
...
대화 분리 결과:`;
  ...
  try {
    ...
    // Validate: result should contain "상담원:" or "고객:"
    ...
    console.error('대화 분리 실패:', error.message);
    return text; // Non-critical, return original
  }
}
...
/**
 * Analyze sentiment with optional team-specific evaluation prompt
 */
...
/**
 * Analyze call outcome (통화 결과 판정)
 * Returns: "성공: 사유" / "실패: 사유" / "보류: 판단불가"
 */
...
결과: 보류: 판단불가`;
  ...
  try {
    ...
    if (match) {
      ...
      // Validate format: should start with 성공/실패/보류
      if (outcome.startsWith('성공') || outcome.startsWith('실패') || outcome.startsWith('보류')) {
        // Ensure colon separator exists
    ...
    console.error('통화 결과 판정 실패:', error.message);
    return null;
  }
}

/**
 * Get default team prompt based on team name pattern
 */
...
/**
 * Full analysis pipeline: conversation formatting + summary + sentiment + customer name + outcome
 * Team-specific prompts applied.
 */
async function analyzeCall(text, teamPrompt, teamName) {
  ...
  // Determine evaluation prompt: custom team prompt > default by team name > null
  const effectivePrompt = teamPrompt || getDefaultTeamPrompt(teamName) || null;

  // Run sequentially to avoid overloading Ollama on low-power CPU (N100)
  try {
    console.log('[Ollama] Step 1/5: 대화 분리...');
    const formattedText = await formatConversation(text);

    console.log('[Ollama] Step 2/5: 요약...');
    const summary = await generateSummary(text);

    console.log('[Ollama] Step 3/5: 감정 분석...');
    const sentiment = await analyzeSentiment(text, effectivePrompt);

    console.log('[Ollama] Step 4/5: 고객명 추출...');
    const customerName = await extractCustomerName(text);

    console.log('[Ollama] Step 5/5: 결과 판정...');
